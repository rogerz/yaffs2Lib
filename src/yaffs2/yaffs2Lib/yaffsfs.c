...
yaffs_Object *yaffsfs_FindDirectory(yaffs_Object *relativeDirectory,const YCHAR *path,YCHAR **name,int symDepth)
{
...
//
yaffs_Object *yaffsfs_FindObject(yaffs_Object *relativeDirectory, const YCHAR *path,int symDepth)
{
	yaffs_Object *dir;
	YCHAR *name;
...
yaffs_DIR *yaffs_opendir(const YCHAR *dirname)
{
	...
	if(obj && obj->variantType == YAFFS_OBJECT_TYPE_DIRECTORY)
	{
		...
		dsc = (yaffsfs_DirectorySearchContext *)YMALLOC(sizeof(yaffsfs_DirectorySearchContext));
		dir = (yaffs_DIR *)dsc;
		if(dsc)
		{
...
int yaffs_deltree(const char *dname)
{
	yaffs_DIR *d;
	yaffs_dirent *de;
	struct yaffs_stat s;
	...
	if(!d)
	{
		T(YAFFS_TRACE_ERROR, ("opendir failed\n"));
	}
	else
	{
		while((de = yaffs_readdir(d)) != NULL)
		{
			...
			switch(s.st_mode & S_IFMT)
			{
				case S_IFREG: 
				case S_IFLNK:
					if(	yaffs_unlink(str) == -1){
						T(YAFFS_TRACE_ERROR, ("unlink %s failed\n", str));
						retVal = -1;
					}
					break;
				case S_IFDIR:
					yaffs_deltree(str); break;
				default:
					T(YAFFS_TRACE_ERROR, ("unknown")); break;
			}
		}
		yaffs_closedir(d);
		if(yaffs_rmdir(dname) == -1){
			T(YAFFS_TRACE_ERROR, ("rmdir %s failed\n", dname));
			retVal = -1;
		}
	}
	return retVal;
...
void yaffs_ls(const char *dname,int recursive)
{
	yaffs_DIR *d;
	yaffs_dirent *de;
	struct yaffs_stat s;
	...
	else
	{
		while((de = yaffs_readdir(d)) != NULL)
		{
			...
			*/
			localtime_r(&s.yst_mtime, &local);
			strftime(str_time, 32, "%Y-%m-%d %H:%M:%S", &local);
			
			printf("%s %s length %d ", str, str_time, (int)s.st_size);
			switch(s.st_mode & S_IFMT)
			{
...
char * yaffs_gets(char *buf, int size, int fd)
{
	char *pos = NULL;
	int readBytes;

	if((readBytes=yaffs_read(fd, buf, size)) <= 0){
		return NULL;
	}
	pos=strchr(buf, '\n');
	if(pos==NULL){
		...
		*pos='\0';
		/* 实际读取的大小 */
		yaffs_lseek(fd, pos-(buf+readBytes)+1, SEEK_CUR);
	}
	/* 回溯文件指针 */
	return buf;
}

int yaffs_puts(const char *buf, int fd)
{
	return yaffs_write(fd, buf, strlen(buf));
}

int yaffs_eof(int fd)
{
	yaffsfs_Handle *h = NULL;
	yaffs_Object *obj = NULL;
	int pos = 0;
	...
	{
		pos = h->position;
		retVal = yaffs_GetObjectFileLength(obj) == pos ? 1:0;
	}
	yaffsfs_Unlock();
	return retVal;
}

int yaffs_copyfile(const char *yaffs_path, const char *dest)
{
	int inh,outh;
	unsigned char buffer[100];
	int ni,no;
	int retVal=0;
	inh = yaffs_open(yaffs_path, O_RDONLY, 0644);
	if(inh < 0){
	   T(YAFFS_TRACE_ERROR, ("open %s failed\n", yaffs_path));
   	   return -1;
	}
	outh = open(dest, 0x201, 0644);
	if(outh < 0){
		T(YAFFS_TRACE_ERROR, ("open %s failed\n", dest));
		close(inh);
		return -1;
	...
	while((ni = yaffs_read(inh,buffer,100)) > 0)
	{
		...
		if(ni != no)
		{
			T(YAFFS_TRACE_ERROR, ("problem copying yaffs file\n"));
			retVal=-1;
		}
	}
